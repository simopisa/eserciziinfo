        ...
        <section>
            <nav id="app-nav" class="app-nav">
                <ul class="links-lvl1 app-nav-main-links links-with-text">
                    ...
                    <li><a href="managment.php" onclick="window.location.href='managment.php'" class="link-lvl1 selected"><i class="fa fa-th-list "></i><span>Elenco</span></a></li>
          ...
          <td  class='text-left' style=\"text-align: center; width:auto\"><input type=\"submit\" value=\"Elimina\" name=\"elimina\" class=\"button-invia info\"></td>
                                ...
                                ?>
                                <?php
                                $query = mysqli_query($connessione, "SELECT user,ragsociale,id, idcommerciale  from negozi");
                                while ($results = $query->fetch_assoc()) {
                                    if ($results["idcommerciale"]==1  ) {
                                        echo "<tr>
                                        <form action='../script/router.php' method='post'>
